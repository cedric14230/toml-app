...
const ItemCard = ({ brand, title, price, stars, tone, note, status, big }: WishItem) => (
  <div className={big ? 'card' : 'card-soft'} style={{
    breakInside: 'avoid', marginBottom: 16,
    opacity: status === 'reserved' ? 0.6 : 1,
    position: 'relative',
  }}>
    <div className={`img img-${tone}`} style={{ height: big ? 320 : 220, position: 'relative' }}>
      {stars === 3 && (
      ...
      {status === 'reserved' && (
        <span className="chip chip-mustard" style={{ position: 'absolute', top: 12, left: 12, fontSize: 11 }}>
    ...
    <div style={{ padding: big ? '16px 18px' : '12px 14px' }}>
      <div className="label" style={{ color: 'var(--t-rose)', marginBottom: 4, fontSize: big ? 11 : 10 }}>
        {brand}
      </div>
      <div className="display-2" style={{ fontSize: big ? 20 : 15, marginBottom: note ? 6 : 8, lineHeight: 1.15 }}>
        {title}
      </div>
      {note && (
        <div style={{
    ...
    <div style={{ padding: '24px 40px 60px' }}>
      ...
      <div style={{
        ...
        <div className="img img-1" style={{
          ...
          <span className="sticker" style={{ position: 'absolute', bottom: 12, left: 12 }}>
            🎄 Noël
          </span>
        </div>
        <div style={{ flex: 1, minWidth: 280 }}>
          ...
          <h1 className="display-2" style={{ fontSize: 48, letterSpacing: '-0.025em', marginBottom: 10 }}>
            Noël{' '}
            <span style={{ color: 'var(--t-rose)', fontStyle: 'italic', fontWeight: 600 }}>2026</span>
          </h1>
          <div style={{
      ...
      <div style={{
        ...
        <div style={{
          ...
          background: 'var(--t-paper)', borderRadius: 999, border: '1px solid var(--t-line)',
        }}>
          <button className="btn btn-ghost btn-sm" style={{ padding: '4px 12px', background: 'transparent', color: 'var(--t-ink-3)' }}>
            <TomlIcon name="grid" size={13} />
            Grille
          </button>
          <button className="btn btn-sm" style={{ padding: '4px 12px', background: 'var(--t-ink)', color: 'var(--t-bg)', border: '1.5px solid var(--t-ink)' }}>
