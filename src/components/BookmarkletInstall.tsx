...
export default function BookmarkletInstall() {
  ...
  useEffect(() => {
    ...
    // Minified bookmarklet: reads OG meta + JSON-LD Product, then redirects to /add-item
    const code =
      '(function(){' +
      ...
      "try{var d=JSON.parse(ss[k].textContent||'');" +
      "var ns=d['@graph']?d['@graph']:(Array.isArray(d)?d:[d]);" +
      'for(var j=0;j<ns.length;j++){' +
      "var n=ns[j];if(n['@type']==='Product'){" +
      "t=t||n.name||'';var im=n.image;" +
      "if(!i){if(typeof im==='string')i=im;else if(im&&im.url)i=im.url;else if(Array.isArray(im)&&im[0])i=typeof im[0]==='string'?im[0]:(im[0].url||'');}" +
      "if(!pr){var o=Array.isArray(n.offers)?n.offers[0]:n.offers;if(o&&o.price)pr=String(o.price);}" +
      'break;}}}catch(e){}}' +
      "window.location.href='" +
      origin +
      "/add-item?'+new URLSearchParams({title:t,image:i,price:pr,sourceUrl:window.location.href}).toString();" +
      '})()'
      ...
      <div className="flex items-start gap-4 p-5 bg-gray-50 rounded-xl border border-gray-200">
        ...
        <div className="flex-1 min-w-0">
          ...
          <p className="text-sm text-gray-500 mb-3">
            ...
            cliquez dessus pour l&apos;ajouter instantanément à votre wishlist.
          </p>

          {/* Bouton bookmarklet à glisser */}
          <div className="flex items-center gap-3">
            <a
              ...
              onClick={(e) => {
                ...
                // Si cliqué sur la page TOML elle-même (et non glissé), on explique
                e.preventDefault()
                alert(
                  'Glissez ce bouton dans votre barre de favoris.\n\n' +
                  'Ensuite, allez sur une page produit et cliquez dessus.'
                )
              }}
